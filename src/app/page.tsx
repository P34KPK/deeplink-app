...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Simple debounce
    // Auto-fetch for everyone (Free & Pro) so we always have a product image
      ...
      fetch(`/api/metadata?t=${Date.now()}`, {
        ...
        .then(data => {
          if (data.image) {
            ...
            // Silent fail for auto-fetch, user can try manual
          }
        })
        .catch(e => {
          console.error(e);
  ...
  }, [inputUrl]); // Removed isPro dependency

  // ... (lines 149-270 unchanged)

  // In generateLink function:
  let chosenImage = undefined;

  // Allow Auto/Scraped Image for EVERYONE as default
  if (imageOption === 'auto' || imageOption === 'default') chosenImage = scrapedImage || undefined;
