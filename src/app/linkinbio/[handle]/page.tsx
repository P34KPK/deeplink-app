...
export default function UserBioPage() {
    ...
    return (
        <div className={`min-h-screen flex flex-col items-center relative overflow-hidden font-sans transition-colors duration-500`} style={{ color: isDark ? 'white' : '#18181b', ...bgStyle }}>
            ...
            <div className={`w-full md:max-w-6xl relative z-10 flex flex-col md:flex-row min-h-screen backdrop-blur-sm border-x shadow-2xl transition-colors duration-500`} style={{ borderColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)', background: isDark ? 'rgba(0,0,0,0.6)' : 'rgba(255,255,255,0.85)' }}>
                ...
                <div className="w-full md:w-[400px] shrink-0 md:h-screen md:sticky md:top-0 md:border-r border-white/10 flex flex-col relative md:justify-center">
                    <div className="pt-4 pb-6 px-6 flex flex-col items-center text-center md:items-start md:text-left">
                    ...
                    {user?.customLinks && user.customLinks.length > 0 && (
                        <div className="w-full flex flex-col gap-2 mt-8 animate-in slide-in-from-bottom-2 fade-in duration-500">
                    ...
                    {/* Branding Footer (Mobile Link) */}
                    <div className="md:hidden pt-8 pb-4 flex justify-center w-full opacity-50">
                        <Link href="/" className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Powered by DeepLinkrs</Link>
                    </div>
                </div>
            </div>

            {/* --- RIGHT COLUMN: CONTENT --- */}
            <div className="flex-1 min-h-screen flex flex-col md:h-screen md:overflow-y-auto custom-scrollbar md:bg-black/5">
                ...
                {/* Links Feed */}
                <div className="flex-1 px-4 pb-32 space-y-3 md:px-8 md:pt-4">
                    {filteredLinks.length === 0 ? (
                        <div className="text-center py-20 flex flex-col items-center">
                            <Search className={`w-12 h-12 mb-4 ${isDark ? 'text-zinc-800' : 'text-zinc-300'}`} />
                            <p className="text-zinc-500 text-sm">No products found matching &quot;{searchQuery}&quot;</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-3 md:gap-4 max-w-5xl">
                            {filteredLinks.map((link, idx) => (
                                ...
                                >
                                    <div className={`
                                        ...
                                        border rounded-[1.25rem] p-1.5 pr-4 md:p-3
                                        transition-all duration-300 ease-out
                                        hover:scale-[1.02] hover:shadow-xl
                                        flex items-center gap-3 md:flex-col md:items-start
                                        ${isDark
                                            ? 'bg-zinc-900 hover:bg-zinc-800/90 border-white/5 hover:border-white/20 hover:shadow-purple-500/5'
                                            : 'bg-white hover:bg-zinc-50 border-zinc-100 hover:border-zinc-200 hover:shadow-zinc-200/50 shadow-sm'}
                                    `}>
                                        {/* Thumbnail */}
                                        <div className={`w-12 h-12 md:w-full md:aspect-[4/3] shrink-0 rounded-2xl overflow-hidden relative ${isDark ? 'bg-zinc-800' : 'bg-zinc-100'}`}>
                                            {link.image ? (
                                                <img src={link.image} alt="" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                                            ) : (
                                        ...
                                        {/* Content */}
                                        <div className="flex-1 min-w-0 py-1 md:w-full">
                                            <h3 className={`font-semibold text-sm truncate pr-2 md:whitespace-normal md:line-clamp-2 transition-colors mb-1 ${isDark ? 'text-zinc-200 group-hover:text-white' : 'text-zinc-800 group-hover:text-black'}`}>
                                                {link.title || 'Amazon Product'}
                                            </h3>
                                            {link.tags && link.tags.length > 0 && (
                                                <div className="text-[10px] text-zinc-500 uppercase tracking-wider md:bg-white/5 md:px-1.5 md:py-0.5 md:rounded md:inline-block">
                                                    {link.tags[0]}
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        {/* Action Icon (Mobile) */}
                                        <div className="md:hidden opacity-0 group-hover:opacity-100 transition-opacity -translate-x-2 group-hover:translate-x-0 duration-300">
                ...
                <div className="hidden md:flex absolute bottom-4 right-6 z-50 pointer-events-none">
                    <Link href="/" className={`pointer-events-auto backdrop-blur border rounded-full px-4 py-1.5 flex items-center gap-2 transition-colors shadow-lg hover:scale-105 active:scale-95 ${isDark ? 'bg-zinc-950/80 border-white/10 hover:bg-white/10' : 'bg-white/80 border-zinc-200 hover:bg-zinc-50'}`}>
