...
export default function BioPageClient({ initialUser, initialLinks }: BioPageClientProps) {
    const [links] = useState<LinkData[]>(initialLinks);
    const [user] = useState<UserProfile>(initialUser);
    const [filteredLinks, setFilteredLinks] = useState<LinkData[]>(initialLinks);
    ...
    // Apply theme on mount if user has preference
    useEffect(() => {
        if (user?.theme === 'default-dark') {
            setTheme('dark');
        }
    }, [user]);
    ...
    return (
        ...
        >
            ...
            <div
                ...
                style={{ background: user?.theme && user.theme !== 'default-dark' ? user.theme : (isDark ? '#050505' : '#fafafa') }}
            >
                ...
                ) : (!user?.theme || user.theme === 'default-dark') && (
                    <>
